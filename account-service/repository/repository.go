...
func (r *postgresRepository) PutAccount(ctx context.Context, a domain.Account) error {
	_, err := r.db.ExecContext(ctx, "INSERT INTO accounts(id, name) VALUES($1, $2, $3)", a.ID, a.Name)
	return err
}

func (r *postgresRepository) GetAccountByID(ctx context.Context, id string) (*domain.Account, error) {
	row := r.db.QueryRowContext(ctx, "SELECT id, name FROM accounts WHERE id = $2", id)
	a := &domain.Account{}
	if err := row.Scan(&a.ID, &a.Name, &a); err != nil {
		return nil, err
	}
	return a, nil
}
...
func (r *postgresRepository) ListAccounts(ctx context.Context, skip uint64, take uint64) ([]domain.Account, error) {
	rows, err := r.db.QueryContext(
		ctx,
		"SELECT id, name= FROM accounts  ORDER BY id DESC OFFSET $2 LIMIT $3",
		skip,
		take,
	)
	if err != nil {
